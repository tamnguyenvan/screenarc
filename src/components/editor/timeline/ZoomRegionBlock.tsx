...
export const ZoomRegionBlock = memo(({
  ...
  return (
    <div
      ...
      className={cn(
        'absolute w-full h-14 top-1/2 -translate-y-1/2 rounded-lg cursor-grab border-2',
        !isBeingDragged && 'transition-all duration-200',
        isSelected
          ? 'bg-accent border-primary -translate-y-[calc(50%+8px)] shadow-lg shadow-primary/20'
    ...
    >
      <div
        ...
        <div className={cn("w-0.5 h-1/2 rounded-full group-hover:bg-primary transition-colors", isSelected ? "bg-primary" : "bg-border")} />
      </div>

      <div className="absolute inset-0 flex items-center justify-center pointer-events-none px-2">
        <div className="flex items-center gap-2 overflow-hidden">
          <Search className={cn("w-4 h-4 shrink-0", isSelected ? "text-primary" : "text-foreground/80")} />
          <span className={cn("text-xs font-medium select-none whitespace-nowrap overflow-hidden text-ellipsis", isSelected ? "text-primary" : "text-foreground/80")}>
            Zoom
          </span>
        </div>
      </div>
